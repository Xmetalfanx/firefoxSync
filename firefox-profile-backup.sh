...
[ -d "${ffProfileFolder}" ] && echo "Proeeding to backup INTIAL firefox profiles" && userPrompt || echo "Firefox profile location does not exit, nothing to do, Exiting script" || exit 

# 2. Check if the destination folder exists, if it doesn't CREATE it
checkDirExistsAndCreate "${ffProfileBackupFolder}"
userPrompt
...
backupIntialProfiles && userPrompt


backupFFProfiles && userPrompt